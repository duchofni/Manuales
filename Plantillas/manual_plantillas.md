...
El módulo **Plantillas** genera configuraciones de router (Cisco y Teldat) a partir de formularios web. Sustituye los antiguos ficheros Excel con macros (`.xltm`) y los programas Python que se usaban antes para las mismas tareas.

Con Plantillas puedes:
...
- Generar los comandos de la **migración temporal de red 69→10** que se está haciendo a los routers del cliente.

Cada configuración generada se **guarda automáticamente** en `/mnt/centros/plantillas/<nemónico>/` para que cualquier operador pueda volver a consultarla más tarde.
...
3. En el panel de Mantenimiento, busca y pulsa la tarjeta **Plantillas**.

> **Nota:** También puedes entrar directamente con la URL `?m=plantillas` añadida al final de la dirección de BDU.

---

## 3. La pantalla principal

Al entrar en Plantillas verás 4 tarjetas (categorías) y un botón **📂 Histórico** arriba a la derecha.

![Panel principal de Plantillas con las 4 tarjetas: Completa, Modificaciones, Traductor y Migracion red 69 a 10](./imagenes/captura-01-panel-principal-de-plantillas-con-las-4-tarjetas.png)
...
Cuando hay que dar de alta un **centro nuevo** (o reemplazar el equipo Teldat de un centro existente) y necesitas la configuración completa lista para pegar en el router.

El submódulo elige automáticamente la plantilla correcta en función de la **sede** y el **rol** que indiques:
...
El formulario tiene unos 35 campos agrupados en bloques. Los marcados con `*` son obligatorios. Algunos campos solo aparecen cuando hay sentido (por ejemplo, los datos de las loopbacks 5G solo se piden si el rol es Principal o 1º Backup).
...
1. Pulsa **🧾 Añadir ACL**. Se abre un modal con dos cuadros de texto.
2. En el cuadro de la izquierda **pega la ACL del cliente** (puede ser Cisco o Teldat, da igual).
3. Pulsa **🔄 Previsualizar**. El sistema:
...
4. Verás el resultado en el cuadro de la derecha + un aviso verde indicando cuántas ACEs se han procesado.
5. Si todo es correcto, pulsa **✅ Aceptar**. El badge cambia a verde con `✅ ACL cargada` y el botón pasa a `✏️ Editar ACL`.

![Modal de ACL FILTRO_LAN_SEDE con la entrada Cisco a la izquierda y la preview Teldat a la derecha](./imagenes/captura-05-modal-acl-filtro-lan-sede-con-preview.png)

> **Importante:** Mientras la ACL no esté cargada, el botón **Generar configuración** no funcionará y aparecerá un aviso.

> Si te equivocas y quieres pegar otra, pulsa **✏️ Editar ACL**: el modal se abre con la última que cargaste para que la corrijas sin tener que volver a pegar todo.

### 4.4 Generar y guardar

Cuando todos los campos obligatorios estén rellenos y la ACL cargada:

1. Pulsa **⚙️ Generar configuración**.
...
Pulsa **🔵 Alta VLAN — Cisco** en el acordeón de Modificaciones.
...
Tras rellenar, **⚙️ Generar configuración** y copia el resultado al router.

### 5.2 Alta VLAN Teldat

Pulsa **🟣 Alta VLAN — Teldat**. La estructura es similar a Cisco, pero con campos extra propios de Teldat: **Sede**, **Servicio**, **Redundancia** y **Secuencia prefix-list**.

**Tipo de sede `FLEXWAN`:**

Si seleccionas sede = **FLEXWAN**, el formulario simplifica los campos:
...
### 5.3 VLANs estándar (autorelleno)

Cuando tecleas una de las **VLANs estándar** del SERGAS, el formulario rellena automáticamente varios campos para ahorrarte trabajo y evitar errores:
...
Útil cuando necesitas trabajar con ACLs sueltas: traducir, renumerar o generar comandos de habilitar / deshabilitar.

### 6.1 Traductor Cisco ⇄ Teldat / Renumerar

Pulsa **🔀 Traductor Cisco ⇄ Teldat / Renumerar** en el acordeón de Traductor.

Verás dos cuadros de texto grandes (entrada a la izquierda, resultado a la derecha) y 3 botones:

| Botón | Qué hace |
|---|---|
| 🔄 **Traducir** | Detecta automáticamente si pegaste Cisco o Teldat y traduce al otro. |
...
![Submodulo Traductor con los textareas de entrada y resultado y los botones Traducir, Crear no entry y Renumerar](./imagenes/captura-10-submodulo-traductor-vista-general.png)

**Pasos típicos:**

1. **Pega** la ACL en el cuadro izquierdo.
2. Pulsa el botón que toca (normalmente **🔄 Traducir**).
3. Mira el resultado en el cuadro derecho. Bajo el texto verde verás cuántas ACEs se han procesado y, si hay algo no entendido, un panel **⚠️ Avisos** con las líneas problemáticas.
4. Pulsa **📋 Copiar** para llevarte el resultado al portapapeles.
5. Si te equivocaste, **🧹 Limpiar** vacía el cuadro de entrada.

> **Cómo se traducen los puertos:**
> El traductor sale con número (ej. `eq 137`) cuando el nombre Cisco es ambiguo (`netbios-*`, `pop2`, `ident`, `cmd`…) y con nombre cuando es universal (`www`, `domain`, `ftp`, `telnet`, `ntp`, `snmp`, `bgp`, `https`, `syslog`, `tacacs`, etc.).

> **ACLs nombradas Cisco ↔ numeradas Teldat:**
> Si la ACL Cisco tiene el nombre `FILTRO_LAN_SEDE`, en Teldat sale como `access-list 1000`. Si es un nombre que el sistema no conoce, sale como `access-list xxx` (lo rellenas a mano). El catálogo de mapeos es ampliable.

### 6.2 Habilitar / Deshabilitar ACL

Pulsa **🔛 Habilitar / Deshabilitar ACL**.

Esta pantalla genera los comandos para **aplicar o quitar una ACL** sobre las interfaces de un router. Pega un fragmento del `running-config` con las interfaces afectadas y el sistema saca los comandos correspondientes.
...
![Submodulo Habilitar Deshabilitar ACL con los textareas y los dos botones](./imagenes/captura-11-submodulo-habilitar-deshabilitar-acl.png)
...
Cuando hay que **migrar un centro** del direccionamiento antiguo (69.x.x.x) al nuevo (10.x.x.x). El submódulo genera los **3 bloques de comandos** que el operador pega en el router en distintas fases:
...
![Formulario Migracion red 69 a 10 con los selectores de perfil y rol, las IPs nuevas y el textarea del show running-config](./imagenes/captura-12-formulario-migracion-69-a-10.png)
...
| DHCP-relay | DHCP Datos y DHCP Voz. En Centros de Salud vienen con valores por defecto que solo cambias si hay excepción. En No SERGAS se autocompletan desde el `show running-config` que pegues. |
| Configuración actual | Pega aquí el `show running-config` completo del equipo |

**Detección automática:**

El sistema detecta el **fabricante** (Cisco / Teldat) y el **modelo** (C1111 / ASR / Teldat) leyendo el show running-config. No tienes que indicarlo.
...
![Resultado de Migracion red 69 a 10 con los tres textareas Preparativos, Migracion y Eliminacion](./imagenes/captura-13-resultado-migracion-69-a-10.png)

Bajo el aviso verde verás:
...
Pulsa el botón **📂 Histórico** arriba a la derecha del panel principal para ver todas las plantillas generadas hasta la fecha.

![Vista del historico con la tabla agrupada por nemonico mostrando fecha, fabricante, tipo, VLAN y usuario](./imagenes/captura-14-vista-del-historico.png)
...
El módulo usa **visibilidad condicional**: los campos solo aparecen cuando tienen sentido según lo que ya hayas elegido. Por ejemplo, el campo **Servicio** solo se muestra cuando la sede es FULL MACROLAN o MACROLAN+VPN-IP; el campo **Loopback 600 del otro EDC** solo aparece en sedes FULL VPN-IP cuando el rol no es 2º Backup. Esto evita rellenar datos que no aplican.

---

### Pego una ACL Cisco en el modal de FILTRO_LAN_SEDE pero el resultado sale con `access-list xxx`. ¿Por qué?

El nombre Cisco que has pegado no está en el catálogo de mapeos. El operador puede:
...
### La migración 69→10 me dice que detectó Cisco pero el equipo tiene 4G. ¿Qué hago?

El rol **Equipo 4G** solo está soportado en Teldat, porque en SERGAS los equipos 4G son siempre Teldat. Si el sistema detecta Cisco con rol 4G, dará error. Revisa que el `show running-config` que pegaste corresponde realmente al equipo 4G del centro.

---

### Generé una plantilla pero olvidé copiarla. ¿La puedo recuperar?

Sí, todas las plantillas generadas se guardan automáticamente en el histórico. Entra en **📂 Histórico**, busca el nemónico del centro y pulsa **Ver** sobre la plantilla concreta. El botón **📋 Copiar** te la lleva al portapapeles otra vez.

---

### En el histórico aparece una plantilla con el campo "Fabricante" o "Tipo" en blanco. ¿Por qué?

Las plantillas generadas antes de abril 2026 con el submódulo de Migración red 69→10 no guardaron esos campos en el sidecar. Las nuevas sí los rellenan correctamente. Si necesitas recuperar una antigua, abre el `.txt` directamente desde Ver y verás la configuración completa.

---

### Pongo el rol de FLEXWAN en Alta VLAN Teldat pero no me deja elegir el servicio. ¿Es un error?

No es un error, es **lo correcto**. En FLEXWAN el servicio es siempre 5G y el rol siempre 2º BACKUP, así que el formulario los oculta y los rellena él. Tú solo tienes que indicar la sede y el resto de campos comunes.
